...
//{"distance":"12","theta":"13"}/n
void json_data_process(char *data)
{
	my_data_json = cJSON_Parse(data);

	if(cJSON_GetObjectItem(my_data_json, "distance"))
	{
		distance = atof(cJSON_GetObjectItem(my_data_json, "distance")->valuestring);
	}

	if(cJSON_GetObjectItem(my_data_json, "theta"))
	{
		theta = atof(cJSON_GetObjectItem(my_data_json, "theta")->valuestring);
	}
