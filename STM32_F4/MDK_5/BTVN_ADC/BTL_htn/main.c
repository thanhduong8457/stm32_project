...
ADC_CommonInitStructure.ADC_TwoSamplingDelay = ADC_TwoSamplingDelay_5Cycles; /*thoi gia tre giua 2 lan lay mau*/ 
...
void DMA2_Stream0_IRQHandler(void) //dat dung ten thi khi co ngat no nhay vao
{
	 uint16_t i,m=0,k=0,f=0;
	
  /* Clear the DMA1_Stream2 TCIF2 pending bit */
	DMA_ClearITPendingBit(DMA2_Stream0, DMA_IT_TCIF2); // xoa co ngat*/
	 for(i=0; i<2998; i=i+3)     //chep data
    {
		...
		}  		// chep 4 byte vo a, chep xong 4 byte vo a
    ...
    txbuff[17]=(int)(volt_2*100-(txbuff[14]-48)*100-(txbuff[16]-48)*10)+48;
	  ...
	  rcv_flag = 1;
		k=k+1;
		 //cho phep DMA --> luon co
		ADC_1=0;ADC_2=0;ADC_3=0;
		DMA_Cmd(DMA2_Stream0, ENABLE); 
...
void DMA1_Stream2_IRQHandler(void)
{
 ...
 for(i=0; i<BUFF_SIZE_Rx; i++)
 {
	...
	T_m= rxbuff[0]*1000+rxbuff[1]*100+rxbuff[2]*10+rxbuff[3]*1;
	DMA_Cmd(DMA1_Stream2, ENABLE);
}